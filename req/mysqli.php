 ...
 * les instructions SQL pour MySQLi
 *
 * @package SPIP\Core\SQL\MySQLi
 */
 ...
 */
function req_mysqli_dist($host, $port, $login, $pass, $db='', $prefixe='') {
	...
	if (!$db) {
		...
		if (defined('_MYSQLI_SET_SQL_MODE') 
...
$GLOBALS['spip_mysqli_functions_1'] = array(
 ...
 */
function spip_mysqli_set_charset($charset, $serveur='',$requeter=true){
...
function spip_mysqli_get_charset($charset=array(), $serveur='',$requeter=true){
	...
	return spip_mysqli_fetch(mysqli_query($connexion['link'], $c), NULL, $serveur);
}
...
function spip_mysqli_query($query, $serveur='',$requeter=true) {
	...
	$query = _mysqli_traite_query($query, $db, $prefixe);
	...
	if ($e = spip_mysqli_errno($serveur))	// Log de l'erreur eventuelle
		$e .= spip_mysqli_error($query, $serveur); // et du fautif
...
function spip_mysqli_explain($query, $serveur='',$requeter=true){
	...
	$query = 'EXPLAIN ' . _mysqli_traite_query($query, $db, $prefixe);
	$r = mysqli_query($link, $query);
 ...
 */
define('_SQL_PREFIXE_TABLE_MYSQLI', '/([,\s])spip_/S');
 ...
 */
function _mysqli_traite_query($query, $db='', $prefixe='') {
	...
	if (!preg_match('/\s(SET|VALUES|WHERE|DATABASE)\s/i', $query, $regs)) {
		...
		if (stripos($suite,"SELECT")!==false) {
		    ...
		    $suite = $r[1] . _mysqli_traite_query($r[2], $db, $prefixe);
			$suite = query_reinjecte_textes($suite, $textes);
		}
	}
	$r = preg_replace(_SQL_PREFIXE_TABLE_MYSQLI, '\1'.$pref, $query) . $suite;

	#spip_log("_mysqli_traite_query: " . substr($r,0, 50) . ".... $db, $prefixe", _LOG_DEBUG);
	return $r;
}
...
function spip_mysqli_listdbs($serveur='',$requeter=true) {
	$dbs = array();
	if ($res = spip_mysqli_query("SHOW DATABASES", $serveur)){
...
function spip_mysqli_create($nom, $champs, $cles, $autoinc=false, $temporary=false, $serveur='',$requeter=true) {
	...
	$res = spip_mysqli_query("SELECT version() as v", $serveur);
	if ($row = mysqli_fetch_array($res)
	 && (version_compare($row['v'],'5.0','>=')))
		spip_mysqli_query("SET sql_mode=''");
	...
	foreach($champs as $k => $v) {
		$v = _mysqli_remplacements_definitions_table($v);
 ...
 */
function _mysqli_remplacements_definitions_table($query){
...
{
	return spip_mysqli_query("SHOW TABLES LIKE " . _q($match), $serveur, $requeter);
}
 ...
 *     Erreur eventuelle
 **/
function spip_mysqli_error($query='', $serveur='',$requeter=true) {
 ...
 **/
function spip_mysqli_errno($serveur='',$requeter=true) {
	$link = $GLOBALS['connexions'][$serveur ? $serveur : 0]['link'];
	$s = mysqli_errno($link);
...
**/
function spip_mysqli_insert($table, $champs, $valeurs, $desc=array(), $serveur='',$requeter=true) {
...
function spip_mysqli_insertq_multi($table, $tab_couples=array(), $desc=array(), $serveur='',$requeter=true) {
	...
	foreach ($tab_couples as $couples) {
		...
		if (count($valeurs)>=100){
			$r = spip_mysqli_insert($table, $cles, join(', ', $valeurs), $desc, $serveur, $requeter);
			$valeurs = array();
		}
	}
	if (count($valeurs))
 ...
 */
function spip_mysqli_update($table, $champs, $where='', $desc=array(), $serveur='',$requeter=true) {
	$set = array();
...
function spip_mysqli_quote($v, $type='') {
	if ($type) {
		if (!is_array($v))
			return spip_mysqli_cite($v,$type);
		// si c'est un tableau, le parcourir en propageant le type
		foreach($v as $k=>$r)
			$v[$k] = spip_mysqli_quote($r, $type);
		return $v;
	}
 ...
 */
function test_rappel_nom_base_mysqli($server_db)
{
 ...
 */
function test_sql_mode_mysqli($server_db){
	$res = sql_select("version() as v",'','','','','','',$server_db);
	$row = sql_fetch($res,$server_db);
	if (version_compare($row['v'],'5.0.0','>=')){
		define('_MYSQLI_SET_SQL_MODE',true);
		return "define('_MYSQLI_SET_SQL_MODE',true);\n";
	}
	return '';
}
